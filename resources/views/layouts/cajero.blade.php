...
<head>
    ...
    <title>Panel de Cajero - Accesorios Led "El Güero"</title>
        ...
        <nav x-data="{ open: false }" class="fixed top-0 left-0 z-40 w-full md:w-72 lg:w-64 h-screen transition-transform duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] bg-gradient-to-b from-blue-800 via-blue-700 to-blue-600 shadow-xl" :class="{'translate-x-0': open || $screen('md'), '-translate-x-full': !open && !$screen('md')}">
            ...
            <div class="h-full px-4 sm:px-5 py-7 sm:py-8 overflow-y-auto custom-scrollbar scrollbar-w-2 scrollbar-track-blue-100 scrollbar-thumb-blue-400 scrollbar-thumb-rounded-full">
                <div class="flex items-center mb-5 pl-2.5">
                    ...
                    <span class="self-center text-xl font-semibold whitespace-nowrap text-white">Panel Cajero</span>
                </div>
                <ul class="space-y-2.5 sm:space-y-3.5 font-medium">
                    <li>
                        <a href="#" class="flex items-center px-4 py-3.5 text-white/90 hover:text-white rounded-xl transition-all duration-300 group {{ Route::is('dashboard') ? 'bg-orange-500/90 shadow-lg font-semibold border-l-4 border-orange-300' : 'hover:bg-blue-600/50' }}">
                            <i class="fas fa-cash-register w-5 h-5 text-white transition duration-75"></i>
                            <span class="ml-3">Nueva Venta</span>
                        </a>
                    </li>
                    <li>
                        <a href="#" class="flex items-center px-4 py-3.5 text-white/90 hover:text-white rounded-xl transition-all duration-300 group {{ Route::is('dashboard') ? 'bg-orange-500/90 shadow-lg font-semibold border-l-4 border-orange-300' : 'hover:bg-blue-600/50' }}">
                            <i class="fas fa-history w-5 h-5 text-white transition duration-75"></i>
                            <span class="ml-3">Historial de Ventas</span>
                        </a>
                    </li>
                    <li>
                        <a href="#" class="flex items-center px-4 py-3.5 text-white/90 hover:text-white rounded-xl transition-all duration-300 group {{ Route::is('dashboard') ? 'bg-orange-500/90 shadow-lg font-semibold border-l-4 border-orange-300' : 'hover:bg-blue-600/50' }}">
                            <i class="fas fa-user-friends w-5 h-5 text-white transition duration-75"></i>
